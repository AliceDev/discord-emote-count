...
class Cassandra extends Database {
    ...
    async recordEmote(emote_id, name) {
        await this.client.execute(queries.insertEmoteName, [ emote_id, name ], { prepare: true });
    }

    async insert(server_id, user_id, sent_at, emote_id, usages) {
        await this.client.execute(queries.insertEmote, [ server_id, user_id, emote_id, sent_at, usages ], { prepare: true });
    }
    ...
    async getEmoteNames(emote_ids) {
        const query = queries.buildFetchEmoteNameQuery(emote_ids.length);
        const params = emote_ids;

        const { rows } = await this.client.execute(query, params, { prepare: true });

        return groupResultsBy(rows, queries.customEmoteIdColumn, queries.customEmoteNameColumn);
    }
}
 ...
 */
function groupResultsBy(objects, keyColumn, valueColumn) {
    return objects.reduce((acc, current) => {
        const key = current[keyColumn];
        const value = current[valueColumn];
        acc[key] = value;
        return acc;
    }, {});
}
