...
queries.createTables = [
    " CREATE TABLE IF NOT EXISTS emotes "
        ...
        + "     usages smallint "
        + " ); ",
    " CREATE TABLE IF NOT EXISTS custom_emote_names "
        + " ( "
...
queries.wipeData = [
    " TRUNCATE emotes; ",
    " TRUNCATE custom_emote_names; ",
];
queries.dropTables = [
    " DROP TABLE IF EXISTS emotes ",
    " DROP TABLE IF EXISTS custom_emote_names ",
];
...
// TODO: Cache the known emotes so we don't need to use IF NOT EXISTS
//      It has a "non negligible" performance hit (https://cassandra.apache.org/doc/latest/cql/dml.html#insert)
queries.insertEmoteName = " INSERT INTO custom_emote_names "
    + " (id, name) VALUES (?, ?) IF NOT EXISTS; ";
queries.buildFetchEmoteNameQuery = (howMany) => {
    const entries = Array(howMany).fill("?");
    const requestParams = entries.join(", ");

    return `SELECT id, name FROM custom_emote_names WHERE id IN (${requestParams}); `;
};
