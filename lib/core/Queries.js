...
queries.customEmoteServerColumn = "server_id";
queries.wipeData = [
    " TRUNCATE emotes; ",
...
queries.insertEmoteMetadata = " INSERT INTO custom_emote_metadata "
    + " (id, name, animated, server_id) VALUES (?, ?, ?, ?); ";
queries.buildFetchEmoteMetadataQuery = (howMany) => {
    const entries = Array(howMany).fill("?");
    const requestParams = entries.join(", ");

    return `SELECT id, name, animated, server_id FROM custom_emote_metadata WHERE id IN (${requestParams}); `;
};
