...
queries.unsetEmoteGuildMetadata = " UPDATE custom_emote_metadata SET "
    + "     guild_id = null "
    + " WHERE id = ?; ";
queries.buildUpdateEmotesGuild = (howMany) => {
    const entries = Array(howMany).fill("?");
    const requestParams = entries.join(", ");

    return `UPDATE custom_emote_metadata SET guild_id = ? WHERE id IN (${requestParams}); `;
};
queries.buildFetchEmoteMetadataQuery = (howMany) => {
    const entries = Array(howMany).fill("?");
    const requestParams = entries.join(", ");

    return `SELECT id, name, animated, guild_id FROM custom_emote_metadata WHERE id IN (${requestParams}); `;
};
...
queries.fetchEntriesSentAt = ' SELECT "id" FROM emotes WHERE '
    ...
    + "     sent_at = ? "
    + " ALLOW FILTERING; ";
queries.buildDeleteEmotes = (howMany) => {
    const entries = Array(howMany).fill("?");
    const requestParams = entries.join(", ");

    return ` DELETE FROM emotes WHERE id IN (${requestParams}); `;
};
