...
class Client {
    ...
    async onReady() {
        ...
        if(config.reportInterval) {
            this.intervals.reportInterval = setInterval(this.clientReporting.bind(this), config.reportInterval);
        }
    }
    ...
    getAverageShardLatency() {
        return _.mean(this.client.shards.map(shard => shard.latency));
    }
