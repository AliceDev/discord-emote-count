...
class Client {
    constructor(database) {
        ...
        const onGuildCreate = this.onGuildCreate.bind(this);
        this.client.on("guildCreate", onGuildCreate);
        const onGuildDelete = this.onGuildDelete.bind(this);
    ...
    async onReady() {
        const guild_ids = await this.database.fetchGuilds();
        const currentGuild_ids = this.client.guilds.map(g => g.id);
        ...
        const addedGuilds = this.client.guilds.filter(g => addedGuild_ids.includes(g.id));
