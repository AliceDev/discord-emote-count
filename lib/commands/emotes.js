...
const emotes = {
    ...
    run: async function(client, m, args) {
        ...
        if (Object.keys(filters).length != args.length) {
            ...
            const results = mapGroupedKeys(rawResults, groupBy, client);
...
const groupByDisplays = {
    emote: id => /[1-9][0-9]{0,19}/g.exec(id) ? `<:emote:${id}>` : id,
    user: id => `<@!${id}>`,
    server: (id, client) => client.guilds.find(g => g.id === id).name || "Not found",
};
 ...
 */
function mapGroupedKeys(results, groupBy, client) {
    const mappingFunc = groupByDisplays[groupBy] || (x => x);
    return Object.entries(results).reduce((acc, [key, value]) => {
        acc[mappingFunc(key, client)] = value;
        return acc;
    }, {});
}
