...
async function buildKeyMap(keys, groupBy, client) {
    ...
    switch (groupBy) {
        case "user":
            ...
            break;
        case "guild":
            mapFunc = key => client.client.guilds.find(g => g.id === key).name || "Not found";
            break;
        case "emote":
            const idsToFetch = keys.filter(key => /[1-9][0-9]{0,19}/g.exec(key));
            const guilds = new Set(client.client.guilds.map(g => g.id));
            ...
            mapFunc = key => {
                ...
                let result = name;
                if (guild_id && guilds.has(guild_id.toString())) {
