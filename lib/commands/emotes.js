...
const { defaultEntriesLimit, maxEntriesLimit, prefix, developerIDs } = require("../../config");
...
const filterRegexes = {
    guild: {
        ...
        regexes: [
            ...
            { regex: /(?:server|guild):here/ig, onMatch: () => "here" },
            { regex: /(?:server|guild):all/ig, onMatch: () => "all" },
        ],
    ...
    run: async function(client, m, args) {
        ...
        if (!filters || (filters.guild && filters.guild.includes("all") && !developerIDs.includes(m.author.id))) {
            ...
            if (entries.length > 0) {
                ...
                const embed = { color: 0x00ff00, description: filtersRecap, fields: [{ name: "Results", value: description, inline: false }] };
                if (groupBy === "emote") {
...
async function buildKeyMap(keys, filters, groupBy, client, currentGuild_id) {
    ...
    switch (groupBy) {
        ...
        case "guild":
            ...
            break;
        case "emote": {
            const idsToFetch = keys.filter(key => idRegex.test(key));
...
function parseFilters(args) {
    ...
    for (const arg of args) {
        let matched = false;
        for (const [filterName, filter] of Object.entries(filterRegexes)) {
