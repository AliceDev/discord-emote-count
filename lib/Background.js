...
class Background {
    ...
    async onChannelUpdate(channel, oldChannel) {
        ...
        const c = await this.database.fetchChannel(channel.id);
        this.queueChannelFromDatabase(c);
    }
...
Database.connect({}).then(() => {
    const background = new Background(process.env.EMOJILYTICS_TOKEN, Database);
    background.connect();
});
