...
class Background {
    ...
    async onReady() {
        if(config.logChannel && this.client.getChannel(config.logChannel)) {
        ...
        for (const guild of this.client.guilds.values()) {
            for (const channel of guild.channels.values()) {
                if (channel instanceof Eris.TextChannel &&
                    ...
                    await this.database.updateChannel(channel.id, { latestUnparsed_id: latestMessage_id });
                }
            }
        }
    ...
    onLogMessage(packet) {
        const logChannel = this.client.getChannel(config.logChannel)
        channelLogger.logToChannel(logChannel, packet);
    }
